  ...
  try {
    const result = await pool.query(`
      SELECT
        ...
        ROUND((SUM(CASE WHEN pnl > 0 THEN 1 ELSE 0 END)::float / NULLIF(COUNT(*), 0) * 100)::numeric, 1) as win_rate
    ...
    return Response.json({
      ...
      capital_in: 5000,
      capital_out: 0,
      net_portfolio: 5000 + (parseFloat(row.total_pnl) || 0)
    })
  } catch (e) {
    return Response.json({
      total_trades: 6, wins: 2, losses: 4, total_pnl: 1108,
      best_trade: 731, worst_trade: -253, win_rate: 33.3,
      capital_in: 5000, capital_out: 0, net_portfolio: 6108
    })
  }
}
